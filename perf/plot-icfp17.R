...
ratePlot <- function(data, cols, ytitle, breaks, labels, lpos) {
  ...
  plot <- ggplot(dataSummary, aes(x=factor(test, levels=cols), y=meanRate, fill=platform)) +
    ...
    ylab(ytitle) +
    # Paper plots use this greyscale fill
    #scale_fill_grey(start=0.9, end=0.4,
    #                name="Implementation",
    #                breaks=breaks,
    #                labels=labels) +
    # Artifact plots in color
...
pdf(file="fig8b-rx.pdf")
print(rxPlot)
dev.off()

pdf(file="fig8a-tx.pdf")
print(txPlot)
dev.off()

pdf(file="fig9a-perfRatio.pdf")
print(perfRatioPlot)
dev.off()

pdf(file="fig9b-blockPerfRatio.pdf")
print(blockPerfRatioPlot)
dev.off()
