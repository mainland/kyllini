...
test_args <- c("--ziria-data",
               "data/ziria-2016-09-28-mainland-ghc710-b5f8a17c.csv",
               "--kzc-data",
               "data/wifi-2016-09-28-perf-a5734919.csv",
               "--kzc-alt-data",
               "data/wifi-sid-2016-09-28-perf-a5734919.csv",
               "--component-data",
               "data/pepm-component-2016-09-28-perf-20592cb4.csv");

test2_args <- c("--combined-data",
               "data/perf-91ad328f.csv");

opt_parser = OptionParser(option_list=option_list);
opt = parse_args(opt_parser);
#opt = parse_args(opt_parser, args=test2_args);
...
rxPlot <- ratePlot(data, rxCols, "Data rate (Msamples/sec)", breaks, labels, c(0.82, 0.85))
txPlot <- ratePlot(data, txCols, "Data rate (Mbits/sec)", breaks, labels, c(0.19, 0.85))
...
if (!is.null(opt$component_data)) {
  ...
  blocksPlot <- ratePlot(data, columns, "Throughput (Msamples/sec)", categories, labels, c(0.85, 0.85)) +
